...
describe('Register', () => {
  beforeEach(() => {
    vi.clearAllMocks();
    (userService.createUser as any) = vi.fn().mockResolvedValue(undefined);
    (ingredientService.getIngredients as any) = vi.fn().mockResolvedValue({
      ingredients: [
        { id: 1, name: 'Tomato' },
        { id: 2, name: 'Onion' },
      ],
  ...
  it('validates when a user does not supply an email address', async () => {
    ...
    await waitFor(() => {
      expect(screen.getByText(/unfortunately, i will need your email to proceed/i)).toBeInTheDocument();
    });
  });
  ...
  it('validates when a user enters an invalid email address', async () => {
    ...
    await waitFor(() => {
      expect(screen.getByText(/oops, looks like your email address is not valid/i)).toBeInTheDocument();
    }, { timeout: 3000 });
  });
  ...
  it('validates when a user tries to submit the registration and did not select at least one ingredient', async () => {
    ...
    await waitFor(() => {
      expect(screen.getByText(/you need at least one ingredient before i can show you what you can cook/i)).toBeInTheDocument();
    });
  ...
  it('confirms that the user can register once a valid email and at least one ingredient is provided', async () => {
    (ingredientService.getIngredients as any).mockImplementation(async (params: any) => {
      const query = params.query?.toLowerCase() || '';
      if (query.includes('tomato') || query === 't' || query === 'to' || query === 'tom' || query === 'toma') {
        return {
